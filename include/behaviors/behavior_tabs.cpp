...
*/
  ...
  struct tabs : public event_handler
  {
    ...
    virtual void attached(HELEMENT he)
    {
      ...
      azurite::string pname = tab_el.get_attribute("panel");
    ...
    dom::element target_tab(HELEMENT he, HELEMENT h_tabs_container)
    {
      ...
      dom::element el = he;
      azurite::string panel_name = el.get_attribute("panel");
    ...
    bool select(azurite::value indexOrName) {
      ...
      if (indexOrName.is_int()) {
        ...
        azurite::string name = indexOrName.get<azurite::string>();
        for (int n = 0; n < int(all_tabs.size()); ++n) {
          dom::element tab = all_tabs[n];
          azurite::string n_name = tab.get_attribute("panel");
          if(name == n_name)
    ...
    std::vector<azurite::string> names() {
      dom::element tabs_el = self;
      auto all_tabs = tabs_el.find_all(":root>.strip>[panel]");
      std::vector<azurite::string> all_names;
