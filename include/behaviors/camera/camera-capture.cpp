...
namespace camera {
  ...
  bool device_list::get_device(const azurite::string& name, com::ptr<IMFActivate>& pActivate)
  {
    for(unsigned n = 0; n < count(); ++n) {
      azurite::string nname;
  ...
  bool device_list::get_device_name(unsigned index, azurite::string& name)
  {
      if (index >= count())
      {
          return false;
  ...
  capture* capture::create_instance( azurite::video_destination* pdst, const azurite::value& source_name_or_index )
  {
  ...
  capture::capture(azurite::video_destination* pdst ) : 
      m_pReader(NULL),
      m_nRefCount(0),
      ...
      m_width(0),
      m_height(0),
      m_colorSpace(azurite::COLOR_SPACE_UNKNOWN),
      dest(pdst)
  {
  }
  ...
  {
      azurite::sync::critical_section _(m_lock);
  ...
  HRESULT ConfigureSourceReader(IMFSourceReader *pReader, azurite::COLOR_SPACE& color_space)
  {
      color_space = azurite::COLOR_SPACE_UNKNOWN;
      ...
      azurite::COLOR_SPACE color_spaces[] = {
        azurite::COLOR_SPACE_NV12, 
        azurite::COLOR_SPACE_YUY2,
        azurite::COLOR_SPACE_RGB32,
        azurite::COLOR_SPACE_RGB24,
        azurite::COLOR_SPACE_IYUV // a.k.a. I420  
      };
      ...
      if(FAILED(hr))
        return hr;
      return color_space != azurite::COLOR_SPACE_UNKNOWN ? S_OK : E_FAIL;
  }
