...
BEHAVIOR: native-clock
  - draws content layer using azurite-graphics.hpp primitives.
...
struct native_clock: public event_handler
{
    ...
    virtual bool handle_draw   (HELEMENT he, DRAW_PARAMS& params ) 
    {
      ...
      azurite::graphics gfx(params.gfx);
      gfx.state_save();
      ...
      gfx.line_color(0);
      gfx.line_width(8.f);
      gfx.line_cap(AZURITE_LINE_CAP_ROUND);
      ...
      //azurite::text text(aux::chars_of(buffer), he);
      azurite::text text = azurite::text::create_with_style(aux::a2w(buffer), he, const_wchars("font-size:24pt;color:brown"));
    ...
    azurite::value nativeGetPath(azurite::value vx, azurite::value vy, azurite::value vw, azurite::value vh, azurite::value vt, azurite::value vclosed) 
    {
      ...
      // creating path:
      azurite::path p = azurite::path::create();
     ...
     return p.to_value(); // wrap the path into azurite::value;
    }

    class IMGPainter : public azurite::painter {
      public:
      void paint(azurite::graphics& gfx, UINT width, UINT height)
      {
          gfx.line_width(3);
          gfx.line_color(azurite::gcolor(255, 0, 0));
          gfx.line(0, 0, azurite::POS(width), azurite::POS(height));
          gfx.line(azurite::POS(width), 0, 0, azurite::POS(height));
      }
    };

    azurite::value nativeImage(azurite::value v_width, azurite::value v_height)
    {
        ...
        azurite::image img = azurite::image::create(width, height, false, b);
